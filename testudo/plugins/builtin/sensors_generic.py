...
Covers sensor_msgs/msg/LaserScan and sensor_msgs/msg/Imu -- two common raw
sensor shapes on a nav stack -- with checks that generalize across both:
NaN/Inf values, a reading that's stuck (identical to the previous message)
or all-zero, values outside a plausible range, and a frame_id that changes
between messages.
"""
...
class GenericSensorPlugin(CheckPlugin):
    """Zero/stuck-value, NaN/Inf, range-plausibility, and frame_id checks for common sensor types."""
    ...
    def msg_types(cls) -> tuple[str, ...]:
        return (_LASER_SCAN_TYPE, _IMU_TYPE)
